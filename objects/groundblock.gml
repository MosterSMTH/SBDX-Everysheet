...
if (c1 && c2 && c3 && c4 && c6 && c7 && c8 && c9) {/*middle fill*/ tile_bake(sheet,lx + 32+bx,408+by,16,16,x,y,999998)}
else if (!c8 && !c4 && !c6 && !c2) {/*lone tile*/ tile_bake(sheet,lx + 88,480,32,32,x-8,y-8,999998)}
else if (!c8 && !c2) {
    if (!c4) {/*platform left*/ tile_bake(sheet,lx + 8,480,24,32,x-8,y-8,999998)}
    else if (!c6) {/*platform right*/ tile_bake(sheet,lx + 64,480,24,32,x,y-8,999998)}
    else {/*platform middle*/ tile_bake(sheet,lx + 32+bx,480,16,32,x,y-8,999998)}
} else if (!c4 && !c6) {
    if (!c8) {/*pole top*/ tile_bake(sheet,lx + 88,384,32,24,x-8,y-8,999998)}
    else if (!c2) {/*pole bottom*/ tile_bake(sheet,lx + 88,440,32,24,x-8,y,999998)}
    else {/*pole middle*/ tile_bake(sheet,lx + 88,408+by,32,16,x-8,y,999998)}
} else {
    if (!c4) {
        if (!c8) {
            if (!c3) {
            ...
            {/*corner top left*/ tile_bake(sheet,lx + 8,384,24,24,x-8,y-8,999998)}
        } else if (!c2) {
            if (!c9) {/*slim corner bottom left*/tile_bake(sheet,lx + 168,408,24,24,x-8,y,999998)} 
            else {/*corner bottom left*/ tile_bake(sheet,lx + 8,440,24,24,x-8,y,999998)}
        } else {//!4 & 8 & 2 & 6
            if (!c9 && c3 ) {tile_bake(sheet,lx + 120,448,24,16,x-8,y,999998)}
             ...
             else {/*left wall*/ tile_bake(sheet,lx + 8,408+by,24,16,x-8,y,999998)}
        }
    } else if (!c6) {
        if (!c8) {
            if (!c1) {/*slimcorner top right*/ tile_bake(sheet,lx + 192,384,24,24,x,y-8,999998)}
             else {/*corner top right*/ tile_bake(sheet,lx + 64,384,24,24,x,y-8,999998)}
        } else if (!c2) {
            if (!c7) {/*slimcorner bottom right*/ tile_bake(sheet,lx + 192,408,24,24,x,y,999998)}
             else {/*corner bottom right*/ tile_bake(sheet,lx + 64,440,24,24,x,y,999998)}
        } else {
            ...
            else {/*right wall*/ tile_bake(sheet,lx + 64,408+by,24,16,x,y,999998)}
        }
    } else { // 6 && 4 
        if (!c8) {// !8 && 6 && 4 
            ...
            else {/*floor*/ tile_bake(sheet,lx + 32+bx,384,16,24,x,y-8,999998)}
        } else if (!c2) { //8
            ...
            else {/*ceiling*/ tile_bake(sheet,lx + 32+bx,440,16,24,x,y,999998)}
        } else { // 8 & 2 & 6 & 4
            if (c7+c9+c1+c3!=3) {
                if (c7+c9+c1+c3!=2){
                    if (c7+c9+c1+c3!=1){
